  ...
  def visit(node: AstNode, arg: Null): Unit = {
    ...
    node match {
      ...
      case fun: AFunDeclaration =>
        unify(fun, TipFunction(fun.args.map(x => x: Var[TipType]), fun.stmts.ret.value))
        if (fun.name == "main") {
          unify(fun, TipFunction(fun.args.map(_ => TipInt()), TipInt()))
        }
      case call: ACallFuncExpr =>
        unify(call.targetFun, TipFunction(call.args.map(x => x: Var[TipType]), call))
      case _: AReturnStmt =>
      case rec: ARecord =>
